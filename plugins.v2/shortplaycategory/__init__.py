...
class ShortPlayCategory(_PluginBase):
    # 插件名称
    plugin_name = "短剧自动分类_改"
    # 插件描述
        ...
        try:
            ...
            with lock:
                ...
                for file in check_files:
                    ...
                    if duration == 0:
                        logger.debug("{file} 时长 {duration} 分钟，跳过短剧分类处理")
                        continue
                ...
                if need_category:
                    ...
                    target_root_path = target_path.parent
                    logger.debug(f"父级路径 {target_path.parent} 作'{transfer_info.target_diritem.name}'的根路径")
                    result = self.__move_files(target_root_path,target_path=target_path)
                    if result:
    ...
    def __move_files(self, target_root_path:Path, target_path: Path) -> bool:
        """
        移动文件到分类目录
        ...
        tv_path = target_path
        # 新的文件目录
        new_path = Path(self._category_dir) / target_path.relative_to(target_root_path)
        ...
        if not new_path.exists():
            ...
            for file in target_path.iterdir():
                ...
                if file.is_file():
                    ...
                    # 整季移动
                    try:
                        dst_path = new_path / file.name
                        logger.debug(f"整季移动,file:{file},dst_path:{dst_path}")
                        if dst_path.is_dir(): # 目标文件夹存在,递归调用
                            logger.debug(f"递归调用移动，改变目标路径： {target_path / file.name}")
                            self.__move_files(target_root_path, target_path / file.name) # 目标路径为季目录
                        else:
                            shutil.move(file, new_path)
                    except Exception as e:
                        logger.error(f"移动文件失败：{e}")
                        return False
            need_rm_path_parts = list(filter(lambda x: x not in ['/','\\'],target_path.relative_to(target_root_path).parts))
            need_rm_path = ''
            if len(need_rm_path_parts) > 0:
                need_rm_path = need_rm_path_parts[0]
            # 删除空目录
            if not need_rm_path:
                pass
            elif not SystemUtils.list_files(target_root_path.joinpath(need_rm_path), extensions=settings.RMT_MEDIAEXT + settings.DOWNLOAD_TMPEXT):
                try:
